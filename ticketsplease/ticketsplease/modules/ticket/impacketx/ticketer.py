...
class TICKETER:
    ...
    def signEncryptTicket(self, kdcRep, encASorTGSRepPart, encTicketPart, pacInfos):
        ...
        # NOTE: Update for ticketsplease
        # Return the raw `kdcRep` as we need to wrap the
        # data via AP_REQ and SPNEGO_NegTokenInit
        return kdcRep, cipher, sessionKey
    ...
    def saveTicket(self, ticket, sessionKey):
        ...
        # NOTE: Add for ticketsplease
        # Since we return a raw ticket from `signEncryptTicket`,
    ...
    def run(self):
        ...
        if ticket is not None:
            ...
            # NOTE: Update for ticketsplease
            # Only save the ticket if we are debugging
            if logging.getLogger().level == logging.DEBUG:
                self.saveTicket(ticket, sessionKey)

            # NOTE: Add for ticketsplease
            # Return the ticket and its associated signing data so that
            # we can properly build the AP_REQ/SPNEGO_NegTokenInit message
